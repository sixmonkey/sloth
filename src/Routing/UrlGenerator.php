 ...
 * URL::asset('css/app.css')                        // https://example.com/.../my-theme/assets/css/app.abc123.css
...
{
    /**
     * Cached webpack manifest (source name → hashed path).
     *
    ...
    /**
     * Get the URL for a theme asset, resolved via webpack manifest.
     *
     * Reads assets/manifest.json (generated by webpack-manifest-plugin)
     * and returns the hashed path. Falls back to assets/{path} when
     * the key is not in the manifest.
     *
     * @param string $path Asset source path (e.g. 'css/app.css').
     *
     * @since 1.0.0
     */
    public function asset(string $path): string
    {
        ...
        if (isset($manifest[$key])) {
            $resolved = ltrim($manifest[$key], '/');

            return $this->app->uri($resolved, 'home');
        }

        return $this->theme('assets/' . $key);
    }

    /**
     * Load and cache the webpack manifest.json.
     *
    ...
    private function loadManifest(): array
    {
        ...
        if (is_file($manifestPath)) {
            ...
            $this->manifest = $contents !== false ? (json_decode($contents, true) ?? []) : [];
        } else {
            $this->manifest = [];
        }
